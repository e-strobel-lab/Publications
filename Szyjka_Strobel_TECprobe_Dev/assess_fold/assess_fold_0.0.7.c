...
#define MAXSEQS 16000	//maximum number of sequences to evaluate
#define MAXLEN 256		//maximum input sequence length
#define MAX_DG_INDX 500
#define MAXNAME 32

#define RNDM_VARS_2_MAKE 3000 //number of variants to make in randomization modes 1-3
#define RNDS_PER_VAR 100      //number of variants per input sequence to make in randomization mode 4
...
int shf = 0;                                    //flag to perform shuffle randomization
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        static struct option long_options[] =
        {
            ...
            {"print-structure", no_argument,        0,  'p'},
            {"verbose",			no_argument,		0,	'v'},	//flag for verbose output
            {0, 0, 0, 0}
        };
        
        c = getopt_long(argc, argv, "s:r:c:pv", long_options, &option_index);
    ...
    if (!rndm) {
        limit = total_variants;
    } else {
        limit = total_variants * RNDS_PER_VAR;
    }
     
    char print_command[MAXLINE];
    ...
    for (i = 0; i < limit; i++) {
        ...
        if (print_structure) {
            sprintf(print_command, "/Users/ericstrobel/Documents/RNAstructure/exe/draw ./tmp.ct %s.ps", fa[i].nm);
...
int mk_ipt_fa_shf(struct fasta_entry * fa_ipt, char * rnd_filename)
{
    ...
    int base_cnt[MAXLEN][5] = {0};        //number of times each base is observed at each position
    ...
    for (i = 0; i < total_variants; i++) {
        ...
        for (s = 0; s < RNDS_PER_VAR; s++) {
            ...
            for (j = 0, k = 0; crnt_len > 0; k++, j++) { //TODO add error checking to ensure not leaving table bounds
                if (!isbase(fa_ipt[i].sq[j])) {
                    ...
                    base_cnt[j][TOT]++;
                    switch (crnt_seq[rnd]) { //length is only incremented if line[j] != '-'
                        case 'A': base_cnt[j][A]++; break;
                        case 'G': base_cnt[j][G]++; break;
                        case 'U': base_cnt[j][U]++; break;
                        case 'C': base_cnt[j][C]++; break;
        ...
        //print_shf_nt_dist(strlen(fa_ipt[i].sq), base_cnt);
    }
...
int count_nts(char *ipt, char *shf)
{
    ...
    printf("A:\t%d\t%d\t%4.1f\n", a_ipt, a_shf, ((float)(a_ipt)/(float)(tot_ipt))*100);
    printf("U:\t%d\t%d\t%4.1f\n", u_ipt, u_shf, ((float)(u_ipt)/(float)(tot_ipt))*100);
    printf("G:\t%d\t%d\t%4.1f\n", g_ipt, g_shf, ((float)(g_ipt)/(float)(tot_ipt))*100);
    printf("C:\t%d\t%d\t%4.1f\n\n", c_ipt, c_shf, ((float)(c_ipt)/(float)(tot_ipt))*100);
...
int print_shf_nt_dist(int len, int (*base_cnt)[5])
{
    ...
    for (i = 0; i < len; i++) {
        printf("%d: %%%4.1fA\t%%%4.1fU\t%%%4.1fG\t%%%4.1fC\n",
               i,
               ((float)(base_cnt[i][A])/(float)(base_cnt[i][TOT])*100),
               ((float)(base_cnt[i][U])/(float)(base_cnt[i][TOT])*100),
               ((float)(base_cnt[i][G])/(float)(base_cnt[i][TOT])*100),
               ((float)(base_cnt[i][C])/(float)(base_cnt[i][TOT])*100));
    }
    printf("\n");
